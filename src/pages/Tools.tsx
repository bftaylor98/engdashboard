          ...
          <button
            ...
            className={cn(
              ...
              searchMode === 'cid'
                ...
                : 'bg-[var(--bg-elevated)] text-[var(--text-secondary)] border border-[var(--border-default)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)]'
            )}
          >
            Search by C-ID
          </button>
            ...
            <div className="max-h-96 overflow-y-auto space-y-2">
              {searchResults.map((result) => (
                <button
                  ...
                  className={cn(
                    'w-full text-left p-4 bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-lg',
                    'hover:bg-[var(--bg-hover)] hover:border-accent/30 transition-all cursor-pointer',
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-semibold text-[var(--text-primary)]">{result.cid}</span>
                      {result.stockQty > 0 && (
                      ...
                      {result.stockQty === 0 && (
                        <span className="px-2 py-0.5 bg-[var(--bg-hover)] text-[var(--text-muted)] text-xs rounded">
                          Out of stock
                        </span>
                      )}
                    </div>
                    ...
                    {result.partNo && (
                      <p className="text-xs text-[var(--text-muted)] font-mono">{result.partNo}</p>
                    )}
                  </div>
                </button>
              ))}
            ...
            <div className="pb-4 border-b border-[var(--border-subtle)] flex items-center justify-between">
              ...
              {selectedFromSearch && (
                <button
                  ...
                  className={cn(
                    'px-3 py-1.5 text-sm bg-[var(--bg-surface)] hover:bg-[var(--bg-hover)] text-[var(--text-secondary)] rounded-lg',
                    'border border-[var(--border-default)] hover:border-[var(--border-strong)] transition-colors',
                    'flex items-center gap-2'
                  )}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {componentInfo.description && (
                <div>
                  ...
                  <p className="text-sm text-[var(--text-primary)]">{componentInfo.description}</p>
                </div>
              )}
              ...
              {componentInfo.partNo && (
                <div>
                  <p className="text-xs text-[var(--text-secondary)] mb-1">Part Number</p>
                  <p className="text-sm text-[var(--text-primary)] font-mono">{componentInfo.partNo}</p>
                </div>
              )}
              ...
              {componentInfo.storageLocation && (
                <div>
                  ...
                  <p className="text-sm text-[var(--text-primary)]">{componentInfo.storageLocation}</p>
                </div>
              )}
            </div>
            ...
            <div className="pt-4 border-t border-[var(--border-subtle)]">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-[var(--bg-surface)] rounded-lg p-4 border border-[var(--border-subtle)]">
                  <p className="text-xs text-[var(--text-secondary)] mb-2">Stock Quantity</p>
                  <p className="text-2xl font-bold text-green-400">{componentInfo.stockQty || 0}</p>
                  <p className="text-xs text-[var(--text-muted)] mt-1">Available in stock</p>
                </div>

                {/* Circulation Quantity */}
                <div className="bg-[var(--bg-surface)] rounded-lg p-4 border border-[var(--border-subtle)]">
                  <p className="text-xs text-[var(--text-secondary)] mb-2">Circulation</p>
                  <p className="text-2xl font-bold text-yellow-400">{componentInfo.circulationQty || 0}</p>
                  <p className="text-xs text-[var(--text-muted)] mt-1">Checked out / in use</p>
                </div>
              </div>
              ...
              (componentInfo.maxStock !== null && componentInfo.maxStock !== undefined) ? (
                <div className="mt-4 pt-4 border-t border-[var(--border-subtle)]">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {componentInfo.minStock !== null && componentInfo.minStock !== undefined && (
                      <div>
                        <p className="text-xs text-[var(--text-secondary)] mb-1">Minimum Stock</p>
                        <p className="text-sm text-[var(--text-primary)]">{componentInfo.minStock}</p>
                      </div>
                    )}
                    {componentInfo.maxStock !== null && componentInfo.maxStock !== undefined && (
                      <div>
                        <p className="text-xs text-[var(--text-secondary)] mb-1">Maximum Stock</p>
                        <p className="text-sm text-[var(--text-primary)]">{componentInfo.maxStock}</p>
                      </div>
                    )}
                  </div>
                </div>
