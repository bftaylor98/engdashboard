...
def parse_arguments():
    ...
    parser.add_argument('--output', choices=['console', 'csv', 'webhook'], default='console', 
                       help='Output format: console (default), csv, or webhook for Power Automate')
    parser.add_argument('--csv-file', type=str, default='ncrs.csv', help='CSV output filename')
    parser.add_argument('--webhook-url', type=str, help='Webhook URL to send NCRs to Power Automate (required when using --output webhook)')
...
def fetch_all_ncrs(session_token: str, config) -> List[Dict]:
    ...
    while True:
        ...
        try:
            resp = make_request_with_retry('POST', url, json={"query": query, "variables": variables}, headers=headers, timeout=20)
            data = resp.json()
            if "data" not in data:
...
def get_work_order_operations(work_order_number: str, session_token: str, config) -> List[Dict]:
    ...
    try:
        resp = make_request_with_retry('POST', url, json={"query": query, "variables": {"workOrderNumber": work_order_number}}, headers=headers, timeout=20)
        data = resp.json()
        if "data" not in data:
        ...
        ops_data = work_orders[0].get("ops", {}).get("records", [])
        return ops_data
    except Exception as e:
...
def get_user_name_by_id(user_id: str, session_token: str, config) -> str:
    ...
    try:
        resp = make_request_with_retry('POST', url, json={"query": query}, headers=headers, timeout=20)
        data = resp.json()
...
def send_to_power_automate(ncrs: List[Dict], start: datetime.datetime, end: datetime.datetime, webhook_url: str, session_token: str, config):
    ...
    try:
        ...
        if response.status_code in [200, 202]:
            ...
            print(f"✅ Successfully sent {len(ncr_payload)} NCRs to Power Automate")
        else:
            logging.error(f"Failed to send to Power Automate. Status: {response.status_code}, Response: {response.text}")
            print(f"❌ Failed to send to Power Automate. Status: {response.status_code}")
...
def main():
    ...
    if args.start_date and args.end_date:
        ...
        start, end = get_monday_noon_window()
        logging.info(f"Using Monday-to-Monday window: {start} to {end} (Eastern Time)")
    ...
    # Print summary statistics
    print_summary_statistics(filtered_ncrs, start, end, session_token, config)
    ...
    if args.output == 'csv':
        ...
        if not args.webhook_url:
            logging.error("Webhook URL not provided. Please use --webhook-url to specify it.")
            sys.exit(1)
